...
final class WP_Embed_FB extends Embed_FB {
	...
	static function get_fbsdk() {
		_deprecated_function('WP_Embed_FB::get_fbsdk()','3.0',"Example: \n use SIGAMI\WP_Embed_FB\FB_API; \n FB_API::instance()->api('') ");
...
final class WP_Embed_FB_Deprecated_API {
	...
	public function api( $string = '', $method = 'GET', $message = [] ) {
		if ( ! class_exists( 'FacebookApiException' ) ) {
			...
			require_once( Plugin::path() . 'deprecated/base_facebook.php' );
		}
		try {
			FB_API::instance()->api( $string, $method, $message );
		} catch ( Exception $e ) {
			throw new FacebookApiException( [ 'error_code'        => $e->getCode(),
	...
	public function setExtendedAccessToken(){
		$extended = FB_API::instance()->extendAccessToken($this->access_token);
