...
add_action( 'admin_notices', 'WP_Embed_FB_Plugin::admin_notices' );
add_action( 'wp_ajax_wpemfb_close_warning', 'WP_Embed_FB_Plugin::wpemfb_close_warning' );
add_action( 'wp_ajax_wpemfb_video_down', 'WP_Embed_FB_Plugin::wpemfb_video_down' );
...
if ( is_admin() ) {
	...
	add_filter( 'plugin_action_links_' . plugin_basename( __FILE__ ), 'WP_Embed_FB_Admin::add_action_link', 10 );
	add_action( 'admin_menu', 'WP_Embed_FB_Admin::add_page' );
	add_action( 'admin_init', 'WP_Embed_FB_Admin::admin_init' );
	add_action( 'admin_enqueue_scripts', 'WP_Embed_FB_Admin::admin_enqueue_scripts', 10, 1 );
	add_action( 'in_admin_footer', 'WP_Embed_FB_Admin::in_admin_footer' );
}

if ( WP_Embed_FB_Plugin::get_option( 'fb_root' ) === 'true' ) {
	add_filter( 'the_content', 'WP_Embed_FB::fb_root', 10, 1 );
}
